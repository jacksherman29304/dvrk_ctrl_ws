...
class Psm1Command(Node):
    def __init__(self):
        super().__init__('psm1_command')

        # CRTK topic names
        namespace = "/CRTK/"
        self.tool = 'psm1'
        self.measured_js_topic = namespace + self.tool + "/measured_js"
        self.measured_cp_topic = namespace + self.tool + "/measured_cp" # Cartesian pose of end-effector relative to base
        self.servo_jp_topic = namespace + self.tool + "/servo_jp"
        self.servo_cp_topic = namespace + self.tool + "/servo_cp"
        self.base_world_topic = namespace + self.tool + "/T_b_w" # PSM Base in World frame
